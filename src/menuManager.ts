	...
	if (file instanceof TFile) {
			...
			menu.addItem((item) => {
				...
				sub.addItem((subItem) => {
					subItem
						...
						.onClick(async () => await plugin.handlePrint(true, file));
				});
			});
		}
	} else {
		...
		menu.addItem((item) => {
			item.setTitle('Print all notes in folder')
				.setIcon('printer')
				.onClick(async () => await printFolder(plugin, file as TFolder));
		});
	}
}
 ...
 */
export function addReadingModeMenuItems(plugin: SmartPrintPlugin, menu: Menu): void {
	menu.addItem((item) => {
		item.setTitle('Print note')
			.setIcon('printer')
			.onClick(async () => await plugin.handlePrint(false));
