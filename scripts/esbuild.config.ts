...
if (!fs.existsSync(manifestPath)) {
	...
	console.log(
		"   If you're building plugin-config itself, use 'tsc' instead",
	);
	process.exit(0);
}
...
async function getBuildPath(isProd: boolean): Promise<string> {
	...
	const useRealVault =
		process.argv.includes("-r") || process.argv.includes("real");
	...
	if (!vaultPath) {
		...
		if (isInObsidianPlugins) {
			...
			console.log(
				`ℹ️  Building in Obsidian plugins folder (in-place development)`,
			);
			return pluginDir;
		} else {
				...
				setup: (build): void => {
					...
					build.onResolve(
						...
						(args) => {
							...
							return {
								path: path.resolve(
									"../obsidian-plugin-config/scripts",
									relativePath,
								),
							};
						},
					...
					build.onEnd(async () => {
						...
						if (isProd) {
							if (
								...
								console.log(
									`Successfully installed in ${buildPath}`,
								);
							} else {
